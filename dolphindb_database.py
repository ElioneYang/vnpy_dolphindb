...
from .dolphindb_script import (
    ...
    CREATE_OVERVIEWTICK_TABLE_SCRIPT
)
...
class DolphindbDatabase(BaseDatabase):
    ...
    def __init__(self) -> None:
        ...
        if not self.session.existsDatabase(self.db_path):
            ...
            self.session.run(CREATE_OVERVIEWTICK_TABLE_SCRIPT)
    ...
    def save_bar_data(self, bars: List[BarData], stream: bool = False) -> bool:
        ...
        # 计算已有K线数据的汇总
        overview_table = self.session.loadTable(tableName="overview", dbPath=self.db_path)
    ...
    def save_tick_data(self, ticks: List[TickData], stream: bool = False) -> bool:
        ...
        appender = ddb.PartitionedTableAppender(self.db_path, "overviewtick", "datetime", self.pool)
        appender.append(df)
    ...
    #     """读取K线数据：应对未知bug版本"""
    #     # 最后是括号的问题，分行的问题？
    #     if symbol not in ['A', 'B', 'C', 'I', 'J', 'L', 'M', 'P', 'T', 'V', 'Y']:
    #         bars = self.__load_bar_data(symbol,exchange,interval,start,end)
    #     else:    
    #         start   = str(np.datetime64(start)).replace("-", ".")
    #         end     = str(np.datetime64(end)  ).replace("-", ".")
    #         dmd     = DolphindbDatabase()
    #         table   = dmd.session.loadTable(tableName="bar", dbPath=dmd.db_path)
    #         df      = table.select('*').where(f'symbol="{symbol}"').where(f"exchange='{exchange.value}'").where(f"interval='{interval.value}'").where(f'datetime>={start}').where(f"datetime<={end}").toDF()
    ...
    def delete_tick_data(
        ...
        # 删除汇总
        table = self.session.loadTable(tableName="overviewtick", dbPath=self.db_path)
        (
            table.delete()
            .where(f"symbol='{symbol}'")
            .where(f"exchange='{exchange.value}'")
    ...
    def get_bar_overview(self,todf=False) -> List[BarOverview]:
        """"查询数据库中的K线汇总信息"""
        table = self.session.loadTable(tableName="overview", dbPath=self.db_path)
        ...
        if todf:overviews = pd.DataFrame([o.__dict__ for o in overviews])
        return overviews

    def get_tick_overview(self,todf=False):
        """"查询数据库中的tick汇总信息"""
        table = self.session.loadTable(tableName="overviewtick", dbPath=self.db_path)
        df: pd.DataFrame = table.select("*").toDF()
